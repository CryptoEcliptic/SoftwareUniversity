    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (input != "end of assignment")
            {
                ...
                {
                    foreach (var kvp in teams.Where(x => x.Name == teamToJoin))
                    {
                        kvp.Members.Add(nameJoiner);
                    }
                }
                input = Console.ReadLine();
            }

            foreach (var kvp in teams.Where(x => x.Members.Count != 0).OrderByDescending(x => x.Members.Count)
                .ThenBy(x => x.Name))
            {
                Console.WriteLine($"{kvp.Name}");
                Console.WriteLine($"- {kvp.Creator}");
                foreach (var member in kvp.Members.OrderBy(x => x))
                {
                    Console.WriteLine($"-- {member}");
                }
            }
            Console.WriteLine("Teams to disband: ");
            foreach (var kvp in teams.Where(x => x.Members.Count == 0).OrderBy(x => x.Name))
            {
                Console.WriteLine($"{kvp.Name}");
            }
        }
