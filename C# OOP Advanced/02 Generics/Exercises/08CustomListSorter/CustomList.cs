        ...
        where T : IComparable<T>
    {
        ...
        public void Sort()
        {
            for (int i = 0; i < currentPosition; i++)
            {
                ...
                {
                    if (arrayAsList[i].CompareTo(arrayAsList[j]) > 0)
                    {
                        T currentElement = arrayAsList[i];
                        arrayAsList[i] = arrayAsList[j];
                        arrayAsList[j] = currentElement;
                    }
                }
            }
        }

        public IEnumerator<T> GetEnumerator() //Implementation of IEnumerable<T>
        {
            for (int i = 0; i < currentPosition; i++)
            {
                yield return this.arrayAsList[i];
            }
        }

        IEnumerator IEnumerable.GetEnumerator()//Implementation of IEnumerable<T>
        {
