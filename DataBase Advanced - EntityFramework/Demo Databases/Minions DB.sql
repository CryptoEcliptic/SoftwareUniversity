...
UPDATE Towns SET Name = Lower(Name) FROM Towns WHERE CountryCode IN (SELECT Id FROM Countries WHERE Name = 'Bulgaria')
Select * from Towns
...
UPDATE Minions SET Name = UPPER(LEFT(Name, 1)) + LOWER(RIGHT(Name, LEN(Name) - 1))
WHERE Id = 3
Update Minions SET Name = 'Bob' Where Id = 3
...
SELECT v.Id, mv.MinionId FROM Villains AS v
